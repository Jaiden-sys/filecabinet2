...
namespace filecabinet
{
    public class FileCabinetCustomService : FileCabinetService
    {
        protected override void ValidateParameters(RecordRequest request)
        {
            ...
            if (request.ArchiveId <= 0 || request.ArchiveId >= 30000)
                throw new ArgumentOutOfRangeException(nameof(request.ArchiveId), "Invalid ArchiveID");

            if (request.Weight <= 0 || request.Weight > 400)
